...
az monitor action-group email add \
            --resource-group $AZURE_ACTION_GROUP_RG \
            --action-group $AZURE_ACTION_TAG_NAME \
...
az consumption budget show --name "$AZURE_BUDGET_NAME" --resource-group "$AZURE_ACTION_GROUP_RG" --output table
